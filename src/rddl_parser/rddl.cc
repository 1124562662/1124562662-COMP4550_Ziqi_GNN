...
void RDDLTask::addObject(std::string const& typeName,
    ...
    // 4550 tmp
    std::cout << "object: " << objectName << " ,type name: " << typeName
              << std::endl;
    std::ifstream infile("objects_file_z");
    std::ofstream taskFile_z;
    std::string taskFileName = "objects_file_z";
    if (!infile.good()) {
        taskFile_z.open(taskFileName.c_str());
    } else {
        taskFile_z.open(taskFileName.c_str(), std::ofstream::app);
    }
    taskFile_z << std::endl;
    taskFile_z << "object-and-type:";
    taskFile_z << std::endl;
    taskFile_z << objectName;
    taskFile_z << std::endl;
    taskFile_z << typeName;
    taskFile_z << std::endl;
    taskFile_z.close();
...
void RDDLTask::addParametrizedVariable(ParametrizedVariable* parent,
    ...
    case ParametrizedVariable::STATE_FLUENT: {
        ...
        for (int i = 0; i < sf->params.size(); i++) {
            patameters_types.push_back((sf->params)[i]->type->name);
        }
        std::vector<std::string> instantiated_patameters;
        for (int i = 0; i < sf->params.size(); i++) {
            instantiated_patameters.push_back((sf->params)[i]->name);
        }
        this->Net2write_helper(sf->variableName, sf->fullName, sf->initialValue,
                               "STATE_FLUENT", patameters_types,
                               instantiated_patameters);
        // 4550
    ...
    case ParametrizedVariable::ACTION_FLUENT: {
        ...
        for (int i = 0; i < af->params.size(); i++) {
            patameters_types.push_back((af->params)[i]->type->name);
        }
        std::vector<std::string> instantiated_patameters;
        for (int i = 0; i < af->params.size(); i++) {
            instantiated_patameters.push_back((af->params)[i]->name);
        }
        this->Net2write_helper(af->variableName, af->fullName, af->initialValue,
                               "ACTION_FLUENT", patameters_types,
                               instantiated_patameters);
        // 4550
    ...
    case ParametrizedVariable::NON_FLUENT: {
        ...
        // 4550
        std::vector<std::string> patameters_types;
        for (int i = 0; i < nf->params.size(); i++) {
            patameters_types.push_back((nf->params)[i]->type->name);
        }
        std::vector<std::string> instantiated_patameters;
        for (int i = 0; i < nf->params.size(); i++) {
            instantiated_patameters.push_back((nf->params)[i]->name);
        }

        this->Net2write_helper(nf->variableName, nf->fullName, nf->initialValue,
                               "NON_FLUENT", patameters_types,
                               instantiated_patameters);
...
NonFluent* RDDLTask::getNonFluent(std::string const& name) {
    ...
    // // 4550
    // std::ifstream infile("non-fluents-file");
    // std::ofstream taskFile;
    // std::string taskFileName = "non-fluents-file";
    // if (!infile.good()) {
    //     taskFile.open(taskFileName.c_str());
    // } else {
    //     taskFile.open(taskFileName.c_str(), std::ofstream::app);
    // }
    // taskFile << name << std::endl;
    // taskFile << std::to_string(nonFluentMap[name]->initialValue) <<
    // std::endl; taskFile.close();
    // // 4550
...
std::vector<StateFluent*> RDDLTask::getStateFluentsOfSchema(
    ...
    // for ( auto it = stateFluentsBySchema.begin(); it !=
    // stateFluentsBySchema.end(); ++it  )
    //        {
    // std::cout << "~~~~"<< std::endl;schema->print(std::cout);std::cout <<
    // "~~~~"<< std::endl; for(int i=0; i<stateFluentsBySchema[it->first].size()
    // ;++i){
    //     stateFluentsBySchema[schema][i]->print(std::cout);
    //                     }
    //        }
