...
void Instantiator::instantiateCPFs() {
    ...
    // CPFDefinitions contains  CDF
    //   first  is :    person-waiting-up
    //   second is :
    //   switch( (and(person-waiting-up(?f) ~(exists_{(?e : elevator) }
    //   [and(and(elevator-at-floor(?e, ?f) elevator-dir-up(?e))
    //   ~(elevator-closed(?e)))])) : $c(1)) ($c(1) :
    //   Bernoulli(ARRIVE-PARAM(?f))) )
...
void Instantiator::instantiateCPF(ParametrizedVariable* head,
    ...
    for (unsigned int i = 0; i < instantiatedVars.size(); ++i) {
        ...
        // instantiatedFormula->print(std::cout);
        // std::cout << std::endl;

        // instantiatedFormula->print(cout); cout<< "       " << endl;
        task->CPFs.push_back(new ConditionalProbabilityFunction(
            instantiatedVars[i], instantiatedFormula));

            std::cout << "print:" << std::endl;
            task->CPFs[ task->CPFs.size()-1]->formula->print(cout);
