...
std::string SearchNode::toString() const {
    ...
    ss << getExpectedRewardEstimate() << " (in " << numberOfVisits
       << " real visits)";
    return ss.str();
}

THTS::THTS(std::string _name, relation_net* rn)
...
void THTS::visitDecisionNode(SearchNode* node) {
    ...
    if (continueTrial(node)) {
        ...
        vector<double> s_now =
            states[stepsToGoInCurrentState].ReadState2Vector();
        appliedActionIndex =
            actionSelection->selectAction(node, s_now, this->thts_rn);
...
void THTS::printConfig(std::string indent) const {
    ...
    case TerminationMethod::TIME_AND_NUMBER_OF_TRIALS:
        Logger::logLine(indent + "Termination method: TIME AND NUM TRIALS",
                        Verbosity::VERBOSE);
        Logger::logLine(indent + "Timeout: " + std::to_string(timeout),
                        Verbosity::VERBOSE);
        Logger::logLine(
    ...
    Logger::logLine(indent + "Trial length: CountDecisionNodes",
                    Verbosity::VERBOSE);
...
void THTS::printStepStatistics(std::string indent) const {
    if (uniquePolicyDueToLastAction) {
        Logger::logLine(indent + "Policy unique due to optimal last action",
                        Verbosity::NORMAL);
    } else if (uniquePolicyDueToRewardLock) {
        Logger::logLine(indent + "Policy unique due to reward lock",
                        Verbosity::NORMAL);
        Logger::logLine(indent + states[stepsToGoInCurrentState].toString(),
                        Verbosity::VERBOSE);
    } else if (uniquePolicyDueToPreconds) {
        Logger::logLine(
        ...
        Logger::logLine(
            ...
            Verbosity::NORMAL);
        Logger::logLine(indent + "Created search nodes: " +
        ...
        if (currentRootNode) {
            ...
            for (size_t i = 0; i < currentRootNode->children.size(); ++i) {
                ...
                if (child) {
                    ...
                    Logger::logLine(indent + "    " + action.toCompactString() +
                                        ": " + child->toString(),
                                    Verbosity::VERBOSE);
                }
            }
...
void THTS::printRoundStatistics(std::string indent) const {
    ...
    if (!MathUtils::doubleIsMinusInfinity(expectedRewardInFirstSolvedState)) {
        Logger::logLine(indent + "Expected reward in first solved state: " +
                            std::to_string(expectedRewardInFirstSolvedState),
                        Verbosity::SILENT);
    }
    Logger::logLine(indent + "Number of trials in initial state: " +
                        std::to_string(numTrialsInInitialState),
                    Verbosity::SILENT);
    Logger::logLine(indent + "Number of search nodes in initial state: " +
                        std::to_string(numSearchNodesInInitialState),
                    Verbosity::SILENT);

    Logger::logLine(indent + "Number of reward lock states: " +
                        std::to_string(numRewardLockStates),
                    Verbosity::NORMAL);
    Logger::logLine(indent +
                        "Number of states with only one applicable action: " +
