...
int ActionSelection::selectAction(SearchNode* node,
    ...
    // std::cout<<"acti str:"
    // +SearchEngine::actionStates[1].toCompactString()<<std::endl;
...
void UCB1ActionSelection::_selectAction(SearchNode* node) {
   ...
   // if (this->mynn->helper_double >= 0) {

 //    predicted = this->mynn->tensor2vector_z(this->mynn->forward(*(this->state_now)));

      //  scale = this->mynn->helper_double;
    //} else {
     //   scale = 0.0;
   // }

    // cout<<"sss"<<to_string(this->mynn->relation_node_output_size)<<endl;
    ...
    for (unsigned int childIndex = 0; childIndex < node->children.size();
       ...
       //   double NeuralNetworkPrior;

            // if (this->mynn->helper_double >= 0) {

       //  NeuralNetworkPrior = predicted[childIndex];

                // double NeuralNetworkPrior_scale =
                //      magicConstant / (double)node->children[childIndex]->numberOfVisits;
                // NeuralNetworkPrior =
                //     NeuralNetworkPrior * NeuralNetworkPrior_scale;
                    
            // } else {
            //     NeuralNetworkPrior = 0.0;
            // }

            // cout<<"predicted::"<<to_string(NeuralNetworkPrior)<<endl;

            // cout<<"expected
            // reward"<<node->children[childIndex]->getExpectedRewardEstimate()<<endl;

            double UCTValue =
             //   magicConstant * NeuralNetworkPrior +
                 node->children[childIndex]->getExpectedRewardEstimate() +
             visitPart;
...
void UCB1ActionSelection::printConfig(std::string indent) const {
    ...
    switch (explorationRate) {
    case LOG:
        Logger::logLine(indent + "Exploration rate: LOG", Verbosity::VERBOSE);
        break;
    case SQRT:
        Logger::logLine(indent + "Exploration rate: SQRT", Verbosity::VERBOSE);
        break;
    case LIN:
        Logger::logLine(indent + "Exploration rate: LIN", Verbosity::VERBOSE);
        break;
    case LNQUAD:
...
void ActionSelection::printStepStatistics(std::string indent) const {
    ...
    double perc =
        static_cast<double>(numExplorationInRoot) /
        static_cast<double>(numExplorationInRoot + numExploitationInRoot);
    Logger::logLine(indent + "Percentage exploration: " + std::to_string(perc),
                    Verbosity::VERBOSE);
...
void ActionSelection::printRoundStatistics(std::string indent) const {
    ...
    indent += "  ";
    Logger::logLine(indent + "Percentage exploration in initial state: " +
