...
ProstPlanner::ProstPlanner(string& plannerDesc)
    ...
    // for(int i=0; i<this->searchEngine->actionFluents.size();++i){
    //     searchEngine->actionFluents[i]->print(std::cout);
    ...
    while (!plannerDesc.empty()) {
        ...
        if (param == "-s") {
            ...
            setSearchEngine(SearchEngine::fromString(value,&(this->rn)));
...
void ProstPlanner::finishSession(double& totalReward) {
    ...
    Logger::logLine(
        ">>> END OF SESSION  -- TOTAL REWARD: " + to_string(totalReward),
        Verbosity::SILENT);

    double avgReward = totalReward / numberOfRounds;
    Logger::logLine(
        ">>> END OF SESSION  -- AVERAGE REWARD: " + to_string(avgReward),
        Verbosity::SILENT);
}
...
void ProstPlanner::initRound(long const& remainingTime) {
    ...
    Logger::logLine(">>> STARTING ROUND " + to_string(currentRound + 1) +
                        " -- REMAINING TIME " +
                        to_string(remainingTime / 1000) + "s",
                    Verbosity::SILENT);
...
void ProstPlanner::initStep(vector<double> const& nextStateVec,
    ...
    Logger::logLineIf(currentState.toString(), Verbosity::VERBOSE,
                      "Current state: " + currentState.toCompactString(),
                      Verbosity::NORMAL);
...
vector<string> ProstPlanner::plan(int& action_result) {
    ...
    action_result =  executedActionIndex;
...
void ProstPlanner::monitorRAMUsage() {
    if (cachingEnabled && (SystemUtils::getRAMUsedByThis() > ramLimit)) {
        ...
        Logger::logLine("CACHING ABORTED IN STEP " +
                            to_string(currentStep + 1) + " OF ROUND " +
                            to_string(currentRound + 1),
                        Verbosity::SILENT);
    }
}
...
void ProstPlanner::printConfig() const {
    ...
    Logger::logLine("  Random seed: " + std::to_string(seed),
                    Verbosity::VERBOSE);
    Logger::logLine("  RAM limit: " + std::to_string(ramLimit),
                    Verbosity::VERBOSE);
    Logger::logLine("  Bit size: " + std::to_string(bitSize),
                    Verbosity::VERBOSE);
