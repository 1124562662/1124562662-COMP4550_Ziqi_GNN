...
SearchEngine* SearchEngine::fromString(string& desc,relation_net* rn) {
    ...
    if (isConfig("THTS")) {
     ...
     //----------zzq--------------------------
      ...
      //  result = new NNRollOut(a_net);
    }
    else if (isConfig("NNselection")) {
        desc = desc.substr(11, desc.size());
      //  result = new NNselection(net);
    }
    else if (isConfig("randomselection")) {
        desc = desc.substr(15, desc.size());
        result = new randomselection();
    }
     else if (isConfig("selection2")) {
        desc = desc.substr(10, desc.size());
       result = new NNselection_two(rn);
    }
    //----------zzq--------------------------
    else {
