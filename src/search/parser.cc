...
void Parser::parseTask(map<string, int>& stateVariableIndices,
    ...
    for (size_t i = 0; i < SearchEngine::numberOfActions; ++i) {
        parseActionState(desc, action_indices);
    }
...
void Parser::parseActionFluent(stringstream& desc) const {
    ...
    //
    std::ifstream infile("sogbofa");
    std::ofstream taskFile;
    std::string taskFileName = "sogbofa";
    if (!infile.good()) {
        taskFile.open(taskFileName.c_str());
    } else {
        taskFile.open(taskFileName.c_str(), std::ofstream::app);
    }
    taskFile << "action" << endl;
    taskFile << name << endl;
    taskFile << index << endl;
    taskFile << "action_end" << endl;
    taskFile.close();
    //
    ...
    (*(this->action_f_s)).push_back(new ActionFluent(index, name, values));
...
void Parser::parseCPF(stringstream& desc, vector<string>& deterministicFormulas,
    ...
    taskFile << "cpf" << endl;
    taskFile << name << endl;
    taskFile << "formula" << endl;
    taskFile << formula << endl;
    taskFile << "formula_end" << endl;
    //
    ...
    if (isProbabilistic) {
        ...
        // cout<<"111111111111aqa" <<endl;
        //      probCPF->formula->print(cout);cout<<"" <<endl;
        ...
        //
        taskFile << "index" << endl;
        taskFile << index + State::numberOfDeterministicStateFluents << endl;
        //
...
void Parser::parseRewardFunction(stringstream& desc) const {
    ...
    taskFile << "rewardfunction" << endl;
    taskFile << formulaAsString << endl;
    taskFile << "rewardfunction_end" << endl;
    taskFile.close();
    //
...
void Parser::parseActionState(
    stringstream& desc, std::map<std::string, int>* action_indices) const {
    int index;
    desc >> index;
    ...
    std::cout << "q" << std::to_string(index) << ";"
              << std::to_string(scheduledActionFluents.size());

    if (index > 0) {
        std::stringstream os;
        scheduledActionFluents[0]->print(os);
        std::string s1 = os.str();
        std::cout << s1 << std::endl;
        std::string::iterator end_pos = std::remove(s1.begin(), s1.end(), ' ');
        s1.erase(end_pos, s1.end());
        (*action_indices)[s1] = index;
    }
